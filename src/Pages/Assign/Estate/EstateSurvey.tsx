  ...
  const ButtonClickHandler = (direction: string) => {
    ...
    if (direction === "next" && surveyType === "land" && process === 6) {
      ...
      if (direction === "next" && checkClick) {
        ...
        setResponses((prev) =>
          prev.concat({
            question: Estate_SurveyList[process].question,
            response: Estate_SurveyList[process].responses[clicked].main,
          })
        );
  ...
  return (
    <Layout>
      <Wrap>
        ...
        />
        <SurveyContentBox>
          <QuestionBox>{Estate_SurveyList[process].question}</QuestionBox>
          <ResponseBox>
            {Estate_SurveyList[process].responses.map((response, index) => {
              return (
                <SurveyResponse
                  index={index}
                  clicked={clicked}
